...
def sobel_filter(image):
    image = grayscale(gaussian_blur(image))
...
def canny(image, low, high):
    image, angles = sobel_filter(image)
...
if __name__ == "__main__":
    # mountain statham text
    image = cv2.imread('images/text.png')
    edges, gradient = canny(image, 0, 50)
    show_results(image, edges)